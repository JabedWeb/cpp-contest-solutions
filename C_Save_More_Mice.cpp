...
, the hole is located at the point n
. All mice are located between the cat and the hole: the i
-th mouse is located at the point xi
 (0<xi<n
...
. If the mouse reaches the hole, it hides (i.e. the mouse will not any more move to any point and will not be eaten by the cat). Then (after that the mouse has finished its move) the cat moves to the right by 1
...
It is guaranteed that the sum of all k
 given in the input doesn't exceed 4⋅105
.

Output
For each test case output on a separate line an integer m
 (m≥0
...
int main() {
    ...
    int count = 0, cat_pos = 0, mice_index = 0, mice_pos = arr[0];

    // Move the cat and count saved mice
    while (mice_index < k && cat_pos < mice_pos) {
        count++;
        cat_pos += (n - mice_pos);
        mice_pos = arr[mice_index++];
        mice_index++;
    }
