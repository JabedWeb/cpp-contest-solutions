...
public class post{
    public static void main(String[] args) throws MalformedURLException, IOException {
        ...
        // this is to be done so that we can surely post anything using this connection
        ...
        // first we get the responsecode from the connection, then we test by printing out the code and the responseMessage from URL server
        ...
        // we DO NOT NEED to use GET method, we just read it using inputstream and strings

        // we need string to get printed. so we need a string−type object, where the new URL content will be saved and later on printed
        InputStreamReader in = new InputStreamReader(conn.getInputStream());
        // This is to so that our connection can receive the new posting from the URL
        BufferedReader buffer = new BufferedReader(in);
        // Now buffer can read from the URL server webpage; used for reading
        StringBuffer fromServer = new StringBuffer();
        // This is the string where each line of the new postings from the URL will be saved
        ...
        // After while loop everything from the webpage is now saved into the stringBuffer fromServer, which can be printed
        buffer.close();
